...
def get_empty_xpath_results_rows(data):
    """
    Get rows where Xpath Result is empty or "0.00" for specific market players, regardless of updated_at date.
    Checks multiple possible column names for Xpath Result with improved detection.
    For market players in the ZERO_VALUE_MARKETS list, "0.00" is considered empty.
    For other market players, only truly empty values are considered.
    """
    empty_results = []
    possible_column_names = ["Xpath Result", "XPath Result", "xpath result", "XPATH RESULT", "Xpath_Result", "xpath_result"]
    
    # Market players that should treat "0.00" as empty values
    ZERO_VALUE_MARKETS = ["Amazon"]
    ...
    logger.info(f"Will also check for '0.00' values, but ONLY for these market players: {ZERO_VALUE_MARKETS}")
    
    zero_value_count = {market: 0 for market in ZERO_VALUE_MARKETS}
    other_market_count = 0
    
    # Check for empty values each row
    for row in data:
        is_empty = False
        matched_column = None
        reason = ""
        market_player = row.get("MarketPlayer", "")
        ...
        for column_name in possible_column_names:
            if column_name in row:
                ...
                if value is None:
                    ...
                    reason = "Zero value"
                    break
                elif isinstance(value, str) and value.strip() == "0.00":
                    # Only consider "0.00" as empty for specified market players
                    if market_player in ZERO_VALUE_MARKETS:
                        is_empty = True
                        matched_column = column_name
                        reason = f"Value is 0.00 ({market_player})"
                        zero_value_count[market_player] += 1
                        break
                    else:
                        # Skip 0.00 for other markets
        ...
        if is_empty:
            ...
            if len(empty_results) <= 3:
                ...
                logger.info(f"Found {reason} in column '{matched_column}' for URL: {price_link}")
    
    logger.info(f"Found {len(empty_results)} rows with empty values or configured '0.00' values out of {len(data)} total rows.")
    
    # Log details about zero value counts by market
    for market, count in zero_value_count.items():
        if count > 0:
            logger.info(f"Included {count} {market} rows with '0.00' value.")
    
    logger.info(f"Skipped {other_market_count} non-configured market rows with '0.00' value.")
    
    # If no empty results were found, this could indicate a problem
    if len(empty_results) == 0 and len(data) > 0:
        logger.warning("⚠️ No qualifying empty rows found. This could indicate an issue with column detection.")
